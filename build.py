...
def fetch_ci_time(file_path):
    url = f"https://api.github.com/repos/acshameless/read-website/commits?path={file_path}&page=1&per_page=1"
    ...
    with open('README.md', 'w') as readme_file, open('RECENT.md', 'w') as recent_file:
        ...
        for index, name in enumerate(filenames):
            if name.endswith('.md'):
                ...
                url = f'https://read.shameless.top/posts/{old_title}'
                title = f'第 {old_title.split("-")[0]} 本 - {old_title.split("-")[1]}'
